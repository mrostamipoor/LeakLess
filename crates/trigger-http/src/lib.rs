...
pub struct CliArgs {
    ...
    #[clap(long = "listen", default_value = "0.0.0.0:3000", value_parser = parse_listen_addr)]
...
impl TriggerExecutor for HttpTrigger {
    ...
    async fn new(engine: TriggerAppEngine<Self>) -> Result<Self> {
        ...
        let component_routes = engine
            ...
            let _another = Command::new("../../target/release/inout")
            //.arg("google.com")
            //.stdout(Stdio::piped())
            .spawn()
            .expect("Failed to start the I/O process");
    ...
    async fn run(self, config: Self::RunConfig) -> Result<()> {
        ...
        println!("\nLeakLess Serving {}", "http://0.0.0.0:3001");
        log::info!("Serving {}", base_url);

        println!("Available Routes:");
        for (route, component_id) in self.router.routes() {
            println!("  {}: {}{}", component_id, "http://0.0.0.0:3001", route);
