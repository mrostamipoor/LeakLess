...
impl<C> RawAppManifestAnyVersionImpl<C> {
    ...
    pub fn as_v1(&self) -> &RawAppManifestImpl<C> {
        ...
        /// Clears the manifest field
        pub fn clear_manifest(&mut self) {
            match self {
                RawAppManifestAnyVersionImpl::V1New { manifest, .. } => manifest.clear(),
                RawAppManifestAnyVersionImpl::V1Old { manifest, .. } => manifest.clear(),
            }
        }

}
/*impl<C> Zeroize for RawAppManifestAnyVersionImpl<C> {
    fn zeroize(&mut self) {
        match self {
            RawAppManifestAnyVersionImpl::V1New { manifest, .. } => manifest.zeroize(),
            RawAppManifestAnyVersionImpl::V1Old { manifest, .. } => manifest.zeroize(),
        }
    }
}

impl<C> Default for RawAppManifestAnyVersionImpl<C>
where
    RawAppManifestImpl<C>: Default,
{
    fn default() -> Self {
        RawAppManifestAnyVersionImpl::V1New {
...
//impl<C> Zeroize for RawAppManifestImpl<C> {
   // fn zeroize(&mut self) {
    //    self.variables.zeroize();
   // }
//} //recieved error
...
impl<C> RawAppManifestImpl<C> {
    /// Clears the fields of RawAppManifestImpl
    pub fn clear(&mut self) {
        // Assuming you have fields like components, info, variables, etc.
        self.components.clear();
        self.info.name.clear();
        self.variables.clear();
        // Add other fields to clear as needed
    }
}
/*use zeroize::Zeroize;
impl<C> Zeroize for RawAppManifestImpl<C> {
    fn zeroize(&mut self) {
        // Zeroize each field in your RawAppManifest
        // Example: self.field1.zeroize();
        //          self.field2.zeroize();
        // ...
        self.info.zeroize();
        ...
        // Example: self.sensitive_field.zeroize();
    }
}*/
/// General application information.
#[derive(Clone, Debug, Deserialize, Serialize)]
...
impl RawAppInformation {
    // Function to reset the fields of RawAppInformation
...
/*impl Zeroize for RawAppInformation {
    fn zeroize(&mut self) {
